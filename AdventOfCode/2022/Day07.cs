...
7214296 k";
    ...
    private static Parser<ICommand> CreateCommandParser()
    {
        ...
        var lsResultLine = Parser.OneOf(
            ...
            Parser.FormattedString($"{Parser.UnsignedInteger} {Parser.Line}",
                m => (ILsEntry)new LsEntryFile(m[0], m[1])));
        var lsCommand =
    ...
    private sealed record LsEntryDirectory(string Name) : ILsEntry
    {
        public INode ToNode()
        {
            return DirectoryNode.CreateEmpty(Name);
        }
    }
    ...
    private sealed record DirectoryNode(Dictionary<string, INode> Children, string Name) : INode
    {
        ...
        public static DirectoryNode CreateEmpty(string name)
        {
            return new(new Dictionary<string, INode>(), name);
        }
        ...
        public DirectoryNode GetChildDirectory(string name)
        {
            if (!Children.TryGetValue(name, out var dir))
                dir = Children[name] = CreateEmpty(name);
    ...
    private class Navigator
    {
        ...
        public static Navigator CreateRoot(DirectoryNode current)
        {
            return new(current, null);
        }

        public Navigator ChangeDirectory(string name)
        {
            return new(Current.GetChildDirectory(name), this);
        }

        public override string ToString()
        {
            return Parent != null ? $"{Parent}/{Current.Name}" : Current.Name;
        }
    }
}