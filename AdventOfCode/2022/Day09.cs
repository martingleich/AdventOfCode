...
public class Day09
{
    ...
    private static IEnumerable<Vec2I> ParseInput(string input)
    {
        ...
        var parserCommands = Parser.FormattedString($"{parserDirection} {Parser.Int32}",
    ...
    {
        public Snake Move(Vec2I step) => SetPosition(Position + step);
        ...
        {
            protected override TailSnake SetPosition(Vec2I newHead) => new (newHead);
...
L 5
R 2", 13)]
    public static int ExecutePart1(string input) => CommonExecute(input, 2);
    [TestCase(@"R 5
U 8
L 8
D 3
...
L 25
U 20", 36)]
    public static int ExecutePart2(string input) => CommonExecute(input, 10);
    private static int CommonExecute(string input, int length) => ParseInput(input)
