...
namespace AdventOfCode._2015
{
    public class Day22_Common
    {
        ...
        record class Spell(int Cost, Func<State, State> Cast, Func<State, bool> CanCastIgnoreCost)
        {
            public static readonly Spell MagicMissile = new(53, state => state with { BossHealth = state.BossHealth - 4 }, state => true);
            public static readonly Spell Drain = new(73, state => state with { BossHealth = state.BossHealth - 2, PlayerHealth = state.PlayerHealth + 2 }, state => true);
            public static readonly Spell Shield = new(113, state => state with { ShieldEffect = 6 }, state => state.ShieldEffect <= 1);
            public static readonly Spell Poison = new(173, state => state with { PoisonEffect = 6 }, state => state.PoisonEffect <= 1);
            public static readonly Spell Recharge = new(229, state => state with { RechargeEffect = 5 }, state => state.RechargeEffect <= 1);
            public static readonly ImmutableArray<Spell> AllActions = ImmutableArray.Create( MagicMissile, Drain, Shield, Poison, Recharge );
        }
        ...
        private static State Step(State state, Spell action, int bossDamage, bool hardMode)
        {
            ...
            state = state with { PlayerMana = state.PlayerMana - spellCost, SpendMana = state.SpendMana + spellCost, History = state.History.Add(action) };
            ...
            if (state.BossHealth <= 0)
                return state;
            // Attack the player
            int playerArmor = state.ShieldEffect > 0 ? 7 : 0;
            int damage = Math.Max(1, bossDamage - playerArmor);
        ...
        record struct BossData(int HitPoints, int Damage) { }
        ...
        public static int Execute(string input, bool hardMode)
        {
            ...
            var minMana = FindMinimum(new State(50, 500, bossData.HitPoints, 0, 0, 0, 0, ImmutableList<Spell>.Empty), null, bossData.Damage, hardMode);
    ...
    {
        public int Execute(string input) => Day22_Common.Execute(input, false);
    }
    [Problem("2015-22-02")]
    public class Day22_Part2
    {
        public int Execute(string input) => Day22_Common.Execute(input, true);
    }
}
