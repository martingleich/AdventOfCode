...
public class Day24
{
    private static IEnumerable<(ImmutableList<int>, ImmutableArray<int>)> PickElementsThatSumTo(int[] values,
        int targetSum)
    {
        ...
        int targetSum)
    {
        foreach (var b in baseCases)
        {
            ...
            for (var i = min_index; i < values.Length; ++i)
            {
                var x = values[i];
    ...
    private static bool CanSplitIntoEqualSumSets(ImmutableArray<int> values, int targetSum, int count)
    {
        ...
        {
            if (n == values.Length) return subSums.All(x => x == targetSum);
            for (var i = 0; i < subSums.Length; ++i)
            {
                subSums[i] += values[n];
    ...
    private static ulong Execute(string input, int parts)
    {
        ...
        var min_first_size = int.MaxValue;
        var min_quantum_entanglement = ulong.MaxValue;
        foreach (var x in PickElementsThatSumTo(values, upper_bound))
        {
            ...
            if (CanSplitIntoEqualSumSets(x.Item2, upper_bound, parts - 1))
            {
                ...
                min_quantum_entanglement = Math.Min(min_quantum_entanglement,
                    x.Item1.Aggregate(1ul, (x, y) => checked(x * (ulong)y)));
            }
        }
