...
public class Day13
{
    public static readonly Parser<(string First, bool Sign, int Mag, string Second)> LineParser =
        ...
        from mag in Parser.UnsignedInteger.ThenFixed(" happiness units by sitting next to ")
        from second in Parser.AlphaNumeric.ThenFixed(".")
        select (first, sign, mag, second);

    public static bool MatchSign(string value)
    {
        return value switch
        {
            "lose" => false,
...
David would gain 41 happiness units by sitting next to Carol.", 330)]
    ...
    public static void LoadData(string inputs, out Dictionary<(string, string), int> infos,
        out ImmutableArray<string> circle)
    {
