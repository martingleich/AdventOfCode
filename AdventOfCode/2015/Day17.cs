...
public class Day17
{
    public int GreedySums(int total, ImmutableArray<int> containers, ImmutableArray<int> remaining, int containerCursor)
    {
        if (total == 0)
            return 1;
        var options = 0;
        for (var i = containerCursor; i < containers.Length; ++i)
...
{
    public MinCount GreedySums(int total, ImmutableArray<int> containers, ImmutableArray<int> remaining,
        int containerCursor, int depth, MinCount agg)
    {
        if (total == 0)
