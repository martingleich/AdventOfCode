...
public class Day18_Common
{
    ...
    private static int Execute(bool[,] initial, int count, bool fixCorners)
    {
        ...
        {
            for (var i = 0; i < initial.GetLength(0); ++i)
            for (var j = 0; j < initial.GetLength(1); ++j)
            {
                var countNeighbours = GetNeighbourCount(initial, i, j);
...
####..",
        4, 4)]
    public static int Execute(string input, int count)
    {
        return Day18_Common.Execute(input, count, false);
    }

    public int Execute(string input)
    {
        return Execute(input, 100);
    }
}
...
####..",
    ...
    public static int Execute(string input, int count)
    {
        return Day18_Common.Execute(input, count, true);
    }
