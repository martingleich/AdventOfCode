...
public class Day01
{
    ...
    private static readonly Func<string, IEnumerable<Step>> InputParser = input =>
        input.Split(',').Select(StepParser.Parse);
    ...
    {
        var pos = InputParser(input)
            .Aggregate(new State(Direction.Forward, Position.Origin), (state, step) => state.Move(step)).Pos;
        return pos.Length;
    }
    ...
    private record struct Direction(int X, int Y)
    {
        ...
        public Position Move(Position position, int count)
        {
            return new(position.X + X * count, position.Y + Y * count);
        }

        public Direction Rotate(Direction dir)
        {
            return new(X * dir.X - Y * dir.Y, X * dir.Y + Y * dir.X);
        }
    }
    ...
    private record struct State(Direction Dir, Position Pos)
    {
        public State Move(Step step)
        {
            ...
            return new State(newDir, newDir.Move(Pos, step.Distance));
        }
        ...
        public IEnumerable<State> MoveStepwise(Step step)
        {
            ...
            for (var i = 0; i < step.Distance; ++i)
            {
                ...
                yield return new State(newDir, newPos);
            }
        }
    }
}