...
public sealed class Hoster
{
    ...
    private static string? TryReadInput(Problem problem, string input)
    {
        return input.StartsWith("//") ? Utils.TryReadAllText(input[2..]) : input;
    }
    ...
    public void Profile(ImmutableArray<KeyValuePair<Problem, string>> requests, int repeatCount)
    {
        ...
        for (var i = 0; i < repeatCount; ++i)
        {
            ...
            {
                watches[id].Start();
                for (var j = 0; j < innerRepeatCount; ++j)
